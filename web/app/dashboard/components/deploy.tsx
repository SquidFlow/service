...
import { kustomizationsData } from '@/app/dashboard/components/applicationTemplateMock';
import { DryRun } from './dryrun';
import { clusters, type ClusterInfo } from '@/app/dashboard/components/destinationClusterMock';
import { getAvailableSecretStores, getSecretStoreDetails } from '@/app/dashboard/components/security';
...
export function DeployForm({ onCancel }: DeployFormProps) {
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${
                cluster.environment === 'SIT' ? 'bg-blue-100 text-blue-700' :
                cluster.environment === 'UAT' ? 'bg-green-100 text-green-700' :
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-6">
        {selectedClusters.map((clusterName) => {
          const clusterInfo = clusters.find((c: ClusterInfo) => c.name === clusterName);
          if (!clusterInfo) return null;

          return (
            <Card key={clusterName} className="shadow-sm bg-gradient-to-br from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 border border-gray-200/50 dark:border-gray-700/50">
                  ...
                  {Object.entries(resourceDescriptions).map(([key, desc]) => (
                    <div key={key} className="space-y-2">
                      ...
                      <div className="w-full px-3 py-2 rounded-md bg-gray-100 dark:bg-gray-800 font-mono text-sm">
                        {clusterInfo.resourceQuota[key as keyof typeof clusterInfo.resourceQuota]}
                      </div>
                    </div>
                  ))}
                </TooltipProvider>
  ...
  const [availableSecretStores, setAvailableSecretStores] = useState<SecretStore[]>([]);
...
spec:
                  ...
                  <SelectItem key={store.name} value={store.name}>
                    <div className="space-y-1">
                      <div className="flex items-center space-x-2">
                        ...
                        <Badge variant="outline" className={getProviderBadgeColor(store.provider)}>
                          {store.provider}
                        </Badge>
  ...
  const getProviderBadgeColor = (provider: string) => {
    const colors = {
      ...
      Azure: 'bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400',
      Vault: 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/20 dark:text-indigo-400',
      CyberArk: 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400'
    };
                  ...
                  <div className="space-y-4">
                            ...
                            <TooltipContent>
                              <div className="space-y-2 max-w-xs">
                                ...
                                <ul className="list-disc pl-4 text-sm">
                                  <li>HTTP(S): http(s)://github.com/user/repo.git</li>
                    ...
                    <div className="flex justify-between items-center">
                      <Button
                        ...
                        disabled={!templateSource.value || !templateSource.targetRevision || isValidating}
                        className={`${
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="flex items-center justify-between space-x-2 p-3 bg-white dark:bg-gray-900 rounded-lg">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-xs text-gray-500">
                          {syncOptions.respectIgnoreDifferences ? 'Enabled' : 'Disabled'}
                        </span>
                        <Switch
                    ...
                    <div className="flex items-center justify-between space-x-2 p-3 bg-white dark:bg-gray-900 rounded-lg">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-xs text-gray-500">
                          {syncOptions.createNamespace ? 'Enabled' : 'Disabled'}
                        </span>
                        <Switch
                    ...
                    <div className="flex items-center justify-between space-x-2 p-3 bg-white dark:bg-gray-900 rounded-lg">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-xs text-gray-500">
                          {syncOptions.applyOutOfSyncOnly ? 'Enabled' : 'Disabled'}
                        </span>
                        <Switch
                    ...
                    <div className="flex items-center justify-between space-x-2 p-3 bg-white dark:bg-gray-900 rounded-lg">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-xs text-gray-500">
                          {syncOptions.pruneLast ? 'Enabled' : 'Disabled'}
                        </span>
                        <Switch
                    ...
                    <div className="flex items-center justify-between space-x-2 p-3 bg-white dark:bg-gray-900 rounded-lg">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-xs text-gray-500">
                          {syncOptions.serverSideApply ? 'Enabled' : 'Disabled'}
                        </span>
                        <Switch
